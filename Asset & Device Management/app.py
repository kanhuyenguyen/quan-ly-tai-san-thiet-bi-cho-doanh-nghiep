...
# Hàm kiểm tra ràng buộc đăng nhập
def validate_username(username):
    ...
    if emoji_pattern.search(username):
        return False, "Tên đăng nhập không được chứa emoji"
    
    # Chỉ cho phép chữ cái, số, dấu gạch dưới, dấu chấm
    if not re.match(r'^[a-zA-Z0-9_.]+$', username):
        return False, "Tên đăng nhập chỉ được chứa chữ cái, số, dấu gạch dưới (_) và dấu chấm (.)"
...
def send_borrow_request():
    ...
    try:
        # Lưu vào bảng BorrowRequests (có thể mượn asset hoặc device)
        cursor.execute("""
            INSERT INTO BorrowRequests (UserID, AssetID, DeviceID, RequestReason, ExpectedBorrowDate, ExpectedReturnDate, Status, RequestDate)
            VALUES (?, ?, ?, ?, ?, ?, N'Chờ xét duyệt', GETDATE())
        """, (user_id, asset_id, device_id, reason, borrow_date, return_date))
        conn.commit()
        conn.close()
...
def get_my_borrow_requests():
    ...
    # SỬA: Lấy từ bảng BorrowRequests (nơi lưu yêu cầu mượn mới)
    cursor.execute("""
        SELECT br.RequestID, br.RequestReason, br.Status, 
               FORMAT(br.RequestDate, 'dd/MM/yyyy HH:mm') as CreatedAt,
               FORMAT(br.ExpectedBorrowDate, 'dd/MM/yyyy') as ExpectedBorrowDate,
               FORMAT(br.ExpectedReturnDate, 'dd/MM/yyyy') as ExpectedReturnDate,
               a.AssetName, d.DeviceName
    ...
    for row in cursor.fetchall():
        # Lấy tên tài sản/thiết bị
        item_name = row[6] if row[6] else (row[7] if row[7] else 'Tài sản/Thiết bị')
        
        # Xử lý ngày bàn giao dự kiến (nếu đã duyệt)
        expected_handover = "---"
        if row[2] == "Đã duyệt":
            try:
                # Nếu có HandoverDate trong bảng BorrowRequests
                cursor2 = conn.cursor()
                cursor2.execute("SELECT HandoverDate FROM BorrowRequests WHERE RequestID = ?", (row[0],))
                handover = cursor2.fetchone()
                if handover and handover[0]:
                    expected_handover = handover[0].strftime('%d/%m/%Y') if hasattr(handover[0], 'strftime') else str(handover[0])
                cursor2.close()
            except:
                expected_handover = "---"
        ...
        requests.append({
            ...
            'borrowDate': row[4] if row[4] else '---',
            'returnDate': row[5] if row[5] else '---'
        })
...
def admin_get_all_borrow_requests():
    ...
    # 1. Lấy từ bảng BorrowRequests (mới)
    cursor.execute("""
        SELECT br.RequestID, br.RequestReason, br.Status, 
               FORMAT(br.RequestDate, 'dd/MM/yyyy HH:mm') as RequestDate,
               FORMAT(br.ExpectedBorrowDate, 'dd/MM/yyyy') as ExpectedBorrowDate,
               FORMAT(br.ExpectedReturnDate, 'dd/MM/yyyy') as ExpectedReturnDate,
    ...
    for row in cursor.fetchall():
        item_name = row[8] if row[8] else (row[9] if row[9] else 'Tài sản/Thiết bị')
        requests.append({
            'id': row[0],
            'assetName': item_name,
            'userName': row[6] if row[6] else 'Nhân viên',
            'department': row[7] if row[7] else '',
            'reason': row[1] if row[1] else '',
            'borrowDate': row[4] if row[4] else '---',
            'returnDate': row[5] if row[5] else '---',
            'status': row[2] if row[2] else 'Chờ xét duyệt',
            'createdAt': row[3] if row[3] else '---'
        })
    
    # 2. Lấy từ bảng Reports (dữ liệu cũ, yêu cầu mượn đã lưu trước đây)
    cursor.execute("""
        SELECT r.ReportID, r.ReportContent, r.Status, 
               FORMAT(r.CreatedAt, 'dd/MM/yyyy HH:mm') as CreatedAt,
               u.FullName as UserName, u.Department
        FROM Reports r
        LEFT JOIN Users u ON r.UserID = u.UserID
        WHERE r.ReportContent LIKE N'%YÊU CẦU MƯỢN%'
        ORDER BY r.CreatedAt DESC
    """)
    
    for row in cursor.fetchall():
        content = row[1]
        match_asset = re.search(r'YÊU CẦU MƯỢN - (.*?)\]', content)
        asset_name = match_asset.group(1) if match_asset else 'Tài sản'
        
        match_reason = re.search(r'Lý do: (.*?)\|', content)
